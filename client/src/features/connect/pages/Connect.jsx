...
import RestTriggers from "../components/RestTriggers";
import ConnectForm from "../components/ConnectForm";
import { useAuth } from "../../../state/hooks/useAuth.jsx"; // adjust path if needed

const Connect = () => {
  const [routes, setRoutes] = useState([]);
  const { user } = useAuth(); // assumes your auth hook provides user
    ...
    <div className="bg-gray-900 text-white rounded-lg p-6 w-[95%] max-w-5xl shadow-lg">
      <div className="flex flex-col md:flex-row gap-4">
        ...
        <div className="flex flex-col w-full md:w-2/3 overflow-y-scroll max-h-screen">
          ...
          {/* Pass user to ConnectForm so it can include userId/workflowId */}
          <ConnectForm setRoutes={setRoutes} user={user} />
        </div>
      </div>
    </div>
  );
